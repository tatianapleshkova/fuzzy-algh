...
// lexicase_confidence
void Lexicase_rules(double *x, int **rules_gen, int *class_rule, double *confid_rules, int col, int num_rules, double **reply, int j, int y)
{
    double max = 0;
    int number_rule_best = -1;
    double checki = 0;
    ...
    {
        checki = confid_rules[number_rule_best];
    }
    reply[j][y] = checki;
}
...
void case_order(int C, int *case_order_mas) // C - number of Cases
{
    // инициализация генератора случайных чисел
    srand(time(NULL));
    ...
    for (int i = C - 1; i >= 1; i--)
    {
        ...
        double tmp2 = case_order_mas[j];
        case_order_mas[j] = case_order_mas[i];
        case_order_mas[i] = tmp2;
    }
}

//вариант +
//индивиды лучшие
// n объектов и их параметр для confidence в качестве cases + правильно ли распознание класса
//если не правильная классифик -> confidence = 0 (35 минута)
/*
            ...
            // for (int crossover_int = 0; crossover_int < 2; crossover_int++)
            {
                ...
                // for (int mutation_int = 0; mutation_int < 3; mutation_int++)
                {
                    ...
                    for (int i = 0; i < kfold; i++)
                    {
                        ...
                        // lexicase
                        int num_cases = train_length;
                        int *case_order_mas = new int[num_cases];
                        double **lexi_conf = new double *[num_cases];
                        for (int j = 0; j < num_cases; j++)
                        {
                            lexi_conf[j] = new double[pop_size];
                        }
                        ...
                        for (int generation = 0; generation < gen; generation++)
                        {
                            ...
                            if (which_selection == 2)
                            {
                                // Lexicase selection
                                /*
                                cases := list of training cases in random order := (fitness, num rules, f-score)?;
                                candidates := population of classifiers := individuals (100)
                                */

                                //сделать двумерным и зайти в Lexicase_rules один раз и собрать данные,
                                //затем ими оперировать, каждый кейс необходимо сортировать ПО УБЫВАНИЮ
                                //создаем массив для case и используем объекты
                                for (int y = 0; y < train_length; y++)
                                {
                                    case_order_mas[y] = y;
                                }

                                //перемешиваем
                                case_order(num_cases, case_order_mas);
                                ...
                                for (int j = 0; j < num_cases; j++)
                                {
                                    ...
                                    for (int y = 0; y < pop_size; y++)
                                    {
                                        //(double* x, int** rules_gen, int* class_rule, double* confid_rules, int col, int num_rules, double reply)
                                        Lexicase_rules(train_data[case_order_mas[j]], pop[y], class_rules[0][y], confid_rules[0][y], columnNumber, number_rules, lexi_conf, j, y); //отправка обучающей выборки
                                    }
                                } //ошибка из-за case_order_mas

                                //сортировка массива lexi_conf с сохранением последовательности y
                                double *lexi_watch = new double[pop_size];
                                ...
                                for (int y = 0; y < num_cases; y++)
                                {
                                    for (int j = 0; j < pop_size - 1; j++)
                                    {
                                        //Поиск наименьшего в первом столбце
                                        double m = lexi_conf[y][j];
                                        int idx = j;
                                        for (int l = j; l < pop_size; l++)
                                        {
                                            if (lexi_conf[y][l] > m) //по убыванию
                                            {
                                                m = lexi_conf[y][l];
                                                idx = l;
                                                // Обмен
                                                swap_rows_two_mas(lexi_conf[y], j, idx, lexi_watch);
                                            }
                                        }
                                    }
                                }

                                for (int bg = 0; bg < pop_size; bg++)
                                {
                                    int best_lexi = 0;
                                    //условие выхода из цикла – это break -> lexi_conf[0] != lexi_conf[1]
                                    //если не решилось, то выбираем случайно из тех, что осталось
                                    for (int y = 0; y < num_cases; y++)
                                    {
                                        if (lexi_conf[case_order_mas[y]][0] == lexi_conf[case_order_mas[y]][1])
                                        {
                                            //ищем дальше
                                        }
                                        else
                                        {
                                            cout << lexi_conf[case_order_mas[y]][0] << "\t";
                                            cout << lexi_conf[case_order_mas[y]][1] << "\t";
                                            best_lexi = lexi_watch[0];
                                            //вытягиваем лучшего индивида
                                            for (int l = 0; l < number_rules; l++)
                                            {
                                                for (int j = 0; j < columnNumber; j++)
                                                {
                                                    pop2[bg][l][j] = pop[best_lexi][l][j];
                                                }
                                                rules_update[1][bg][l] = 1;
                                                class_rules[1][bg][l] = class_rules[0][best_lexi][l];
                                                active_rules[1][bg][l] = active_rules[0][best_lexi][l];
                                                confid_rules[1][bg][l] = confid_rules[0][best_lexi][l];
                                                weight_rules[1][bg][l] = weight_rules[0][best_lexi][l];
                                            }
                                            break;
                                        }
                                    }
